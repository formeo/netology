...
# Средняя оценка за домашнии работы и экзамены для своего пола
def get_avg_group_hw_exm_sub_sex(sex,type):
   sum_avg = 0
   sum_sex = 0
   for enum in students:
       if enum['sex'] == sex:
...
# Средняя оценка за домашнии работы и экзамены для своего експириенса
def get_avg_group_hw_exm_sub_exp(exp,type):
    sum_avg = 0
    sum_exp = 0
    for enum in students:
...
def get_avg_group():
    print("Средняя оценка за домашние задания у мужчин:",get_avg_group_hw_exm_sub_sex('male','hw'))
    print("Средняя оценка за экзамен у мужчин",get_avg_group_hw_exm_sub_sex('male','exm'))
    print("Средняя оценка за домашние задания у женщин:", get_avg_group_hw_exm_sub_sex('female','hw'))
    print("Средняя оценка за экзамен у женщин",get_avg_group_hw_exm_sub_sex('female','exm'))
    print("Средняя оценка за домашние задания у студентов с опытом:",get_avg_group_hw_exm_sub_exp(1,'hw'))
    print("Средняя оценка за экзамен у студентов с опытом",get_avg_group_hw_exm_sub_exp(1,'exm'))
    print("Средняя оценка за домашние задания у студентов без опыта:",get_avg_group_hw_exm_sub_exp(0,'hw'))
    print("Средняя оценка за экзамен у студентов без опыта",get_avg_group_hw_exm_sub_exp(0,'exm'))

def get_best_student():
    bs={}
    for i,enum in enumerate(students):
        score=round((0.6 * sum(enum['homework'])) / len(enum['homework']) + 0.4 * (enum['exam']),2)
        bs[i]=score 
        
    max_value = max(bs.values())  
    max_keys = [k for k, v in bs.items() if v == max_value]      
    bestStudents=[students[x] for x in max_keys]

    if len(max_keys) > 1:
        print(("Лучшие студенты: {0} с интегральной оценкой {1}").format( ' , '.join([enum['name']+' '+enum['surname']  for enum in bestStudents]),max_value))
    else:
        print(("Лучшие студент: {0} с интегральной оценкой {1}").format(''.join([enum['name']+' '+enum['surname']  for enum in bestStudents]),max_value))
